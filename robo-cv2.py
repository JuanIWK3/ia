...
ser = serial.Serial('/dev/ttyUSB0', 9600, timeout=1)  # Use correct serial port on Linux
...
while True:
    ...
    if contours:
        ...
        # Determine if the ball is on the left, right, or centered
        frame_center = frame.shape[1] // 2
        center_offset = center_x - frame_center

        if center_offset < -50:
            print("Ball is on the left (soft turn)")
            # Send softer left turn signal
            ser.write(b'L')
        elif center_offset > 50:
            print("Ball is on the right (soft turn)")
            # Send softer right turn signal
            ser.write(b'R')
        else:
            print("Ball is centered, moving forward")
            # Move forward when ball is centered
            ser.write(b'F')
    else:
        # If no ball is detected, stop the robot
        print("Cannot see the ball, stopping")
        ser.write(b'S')
